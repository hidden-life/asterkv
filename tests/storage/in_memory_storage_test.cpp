...
namespace {
    ...
    [[nodiscard]] bool testSupportsConcurrentAccess() {
    AsterKV::Storage::InMemoryStorage storage;

    constexpr int threadCount = 4;
    constexpr int operationsPerThread = 250;

    std::vector<std::thread> workers;

    for (int threadIndex = 0; threadIndex < threadCount; ++threadIndex) {
        workers.emplace_back([&storage, threadIndex]() {
            for (int operationIndex = 0; operationIndex < operationsPerThread; ++operationIndex) {
                const std::string key =
                    "thread_" + std::to_string(threadIndex) + "_key_" +
                    std::to_string(operationIndex);

                const std::string value =
                    "value_" + std::to_string(threadIndex) + "_" +
                    std::to_string(operationIndex);

                static_cast<void>(storage.set(key, value));
                static_cast<void>(storage.exists(key));
                static_cast<void>(storage.get(key));
            }
        });
    }
    ...
    return storage.size() ==
           static_cast<std::size_t>(threadCount * operationsPerThread);
}
...
int main() {
    ...
    if (!testSupportsConcurrentAccess()) {
        return 1;
    }
